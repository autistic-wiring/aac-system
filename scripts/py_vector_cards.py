#!/usr/bin/env python3
"""
Pure Python 512x512 High-Res Anti-Aliased Vector WebP Animation Generator.
Renders all 20 core AAC cards with smooth motion, bold line-art, 512x512 resolution,
and 100% clean alpha transparency in ~3 seconds total.
"""
...
SCALE = 1024 # Render at 1024x1024 and downsample to 512x512 for super-sampled AA
OUT_SCALE = 512
...
def render_hi(frame_idx, total_frames=60):
    # Waving hand gesture
    img = create_super_canvas()
    draw = ImageDraw.Draw(img, "RGBA")
    
    t = frame_idx / total_frames
    angle = math.sin(t * 4 * math.pi) * 22.0 # wave back & forth
    
    # Create hand image and rotate
    ...
    # Rotate around wrist
    rotated = hand_img.rotate(angle, resample=Image.Resampling.BICUBIC, center=(cx, cy + 120))
    img.alpha_composite(rotated)
    return downsample(img)

def render_yes(frame_idx, total_frames=60):
    # Green checkmark expanding with a bounce
    img = create_super_canvas()
    draw = ImageDraw.Draw(img, "RGBA")
...
def render_no(frame_idx, total_frames=60):
    # Red cross mark shaking
    img = create_super_canvas()
    draw = ImageDraw.Draw(img, "RGBA")
...
def render_pointing(frame_idx, total_frames=60, color=(255, 235, 59, 255)):
    # Pointing finger moving vertically
    img = create_super_canvas()
    draw = ImageDraw.Draw(img, "RGBA")
...
def render_help(frame_idx, total_frames=60):
    # Two hands helping (green verb)
    img = create_super_canvas()
    draw = ImageDraw.Draw(img, "RGBA")
...
def render_eat(frame_idx, total_frames=60):
    # Plate + Fork + Spoon eating
    img = create_super_canvas()
    draw = ImageDraw.Draw(img, "RGBA")
    
    t = frame_idx / total_frames
    rot = math.sin(t * 4 * math.pi) * 15.0
...
def render_drink(frame_idx, total_frames=60):
    # Cup tilting
    img = create_super_canvas()
    ...
    rotated = cup_img.rotate(angle, resample=Image.Resampling.BICUBIC, center=(cx, cy + 100))
    img.alpha_composite(rotated)
    return downsample(img)

def render_wait(frame_idx, total_frames=60):
    # Stop Octagon
    img = create_super_canvas()
    draw = ImageDraw.Draw(img, "RGBA")
...
def render_washroom(frame_idx, total_frames=60):
    # Restroom symbol (orange noun)
    img = create_super_canvas()
    draw = ImageDraw.Draw(img, "RGBA")
...
def render_more(frame_idx, total_frames=60):
    # Plus sign pulsing (blue adjective)
    img = create_super_canvas()
    draw = ImageDraw.Draw(img, "RGBA")
...
def render_all_done(frame_idx, total_frames=60):
    # Checkered flag / Done wave (pink social)
    img = create_super_canvas()
    draw = ImageDraw.Draw(img, "RGBA")
    
    t = frame_idx / total_frames
    rot = math.sin(t * 4 * math.pi) * 12.0
...
RENDERERS = {
    ...
    'yes': render_yes,
    'no': render_no,
    'me': lambda f: render_pointing(f, color=(255, 235, 59, 255)),
    'me2': lambda f: render_pointing(f, color=(255, 235, 59, 255)),
    'i': lambda f: render_pointing(f, color=(255, 235, 59, 255)),
    'my_turn': lambda f: render_pointing(f, color=(255, 235, 59, 255)),
    'your_turn': lambda f: render_pointing(f, color=(255, 235, 59, 255)),
    'want': render_help,
    'give': render_help,
    'help': render_help,
    'i_eat': render_eat,
...
def build_card_webp(card_id, render_fn, out_dir, fps=30, total_frames=60):
    tmp_dir = Path(f"/tmp/pyvec_{card_id}")
    if tmp_dir.exists():
        shutil.rmtree(tmp_dir)
    tmp_dir.mkdir(parents=True, exist_ok=True)

    for f in range(total_frames):
        img = render_fn(f, total_frames)
    ...
    print(f"  ✓ [{card_id:10s}] -> {webp_path} ({duration:.2f}s, {total_frames} frames)", flush=True)
...
def main():
    ...
    print("Generating 512x512 crisp vector WebP animations for all 20 cards...", flush=True)
    for card_id, fn in RENDERERS.items():
        durations[card_id] = build_card_webp(card_id, fn, out_dir, fps=30, total_frames=60)
