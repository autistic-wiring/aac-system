#!/usr/bin/env python3
"""
Native Pillow Animated WebP Generator (No FFmpeg, No Temp Files).
Renders 512x512 high-res vector animations for all 20 core AAC cards in < 2 seconds.
"""
...
def main():
    ...
    print("Generating 512x512 crisp vector WebP animations directly via Pillow...", flush=True)
    for card_id, fn in RENDERERS.items():
        frames = [fn(f, 30) for f in range(30)] # 30 frames at 30fps = 1.0s loop
        webp_path = out_dir / f"{card_id}.webp"
        
        # Save directly using Pillow's native animated WebP writer
        frames[0].save(
            webp_path,
            format="WEBP",
            save_all=True,
            append_images=frames[1:],
            duration=33, # ~30 fps (33.3ms per frame)
            loop=0,
            lossless=True,
            quality=90
        )
